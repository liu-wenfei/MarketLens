...
def test_episode01_record_remains_exactly_frozen() -> None:
    record_path = REPO_ROOT / EPISODE01_RECORD_RELATIVE_PATH
    assert _sha256(record_path) == EXPECTED_EPISODE01_RECORD_SHA256
    assert load_episode01_freeze_record(REPO_ROOT)["episode_id"] == "marketlens-canonical-episode-v1-e01"
    assert validate_episode01_freeze_record(REPO_ROOT)["status"] == "PASS"


def test_tracked_episode02_record_has_exact_frozen_hash() -> None:
    record_path = REPO_ROOT / EPISODE02_RECORD_RELATIVE_PATH
    assert record_path.is_file()
    assert _sha256(record_path) == EXPECTED_EPISODE02_RECORD_SHA256


def test_tracked_episode02_record_semantics_pass() -> None:
    record = load_episode02_freeze_record(REPO_ROOT)
    validate_record_semantics(record)


def test_episode02_acceptance_is_technical_not_outcome_conditioned() -> None:
    record = load_episode02_freeze_record(REPO_ROOT)
...
def test_episode02_identity_and_outputs_are_frozen() -> None:
    record = load_episode02_freeze_record(REPO_ROOT)
    assert record["producer_identity"]["git_commit"] == "e2a3e57a008aa3e9744d447047637dcffc4e3d7c"
    ...
    assert record["outputs"]["agent_world_db"]["sha256"] == (
        "577aedbe7f5d07d6fd573e2614275ac99ee804d68d38b303fe9c590c2759efbd"
    )
    assert record["outputs"]["forum_db"]["sha256"] == (
        "b4c1fcd260cf8a84bf8860c8de09c1ede30a7d95ffcb92a81edce67eb5b9fb0b"
    )


def test_episode02_record_hash_guard_rejects_modified_copy(tmp_path: Path) -> None:
    # Include Episode 01 because the shared module intentionally supports both tracked slots.
    e01 = tmp_path / EPISODE01_RECORD_RELATIVE_PATH
    e01.parent.mkdir(parents=True, exist_ok=True)
    e01.write_bytes((REPO_ROOT / EPISODE01_RECORD_RELATIVE_PATH).read_bytes())
    target = tmp_path / EPISODE02_RECORD_RELATIVE_PATH
    target.write_bytes((REPO_ROOT / EPISODE02_RECORD_RELATIVE_PATH).read_bytes())
    assert load_episode02_freeze_record(tmp_path)["episode_id"] == "marketlens-canonical-episode-v1-e02"
    target.write_text(target.read_text(encoding="utf-8") + "\n", encoding="utf-8")
    with pytest.raises(RuntimeError, match="hash mismatch"):
        load_episode02_freeze_record(tmp_path)


def test_episode02_local_evidence_absence_is_portable_not_failure(tmp_path: Path) -> None:
    record = load_episode02_freeze_record(REPO_ROOT)
...
def test_episode02_partial_local_evidence_is_rejected(tmp_path: Path) -> None:
    record = json.loads(json.dumps(load_episode02_freeze_record(REPO_ROOT)))
    agent_path = tmp_path / record["outputs"]["agent_world_db"]["path"]
    agent_path.parent.mkdir(parents=True)
    agent_path.write_bytes(b"partial")
    with pytest.raises(RuntimeError, match="Partial local Episode 02 formal evidence"):
        validate_local_evidence_if_present(tmp_path, record)


def test_episode02_complete_local_evidence_can_be_cross_checked(tmp_path: Path) -> None:
    record = json.loads(json.dumps(load_episode02_freeze_record(REPO_ROOT)))
    agent_path = tmp_path / record["outputs"]["agent_world_db"]["path"]
    ...
    agent_path.write_bytes(b"agent2")
    forum_path.write_bytes(b"forum2")
...
def test_top_level_episode02_validator_passes_on_tracked_record() -> None:
