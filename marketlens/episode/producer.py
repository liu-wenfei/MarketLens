"""Phase 13D formal canonical-episode producer orchestration.
...
PRODUCER_CONTRACT_SHA256 = "14db0ae7a525ef464975f7ba4da69d98eb8ffd4058d491555a32ee25f92a9126"
PRODUCER_CONTRACT_STATUS = "formal_producer_contract_frozen"
PRODUCER_CONTRACT_VERSION = "1.0"
FORMAL_EXECUTION_BANNER = (
    "FORMAL / PHASE 13D CANONICAL EPISODE SLOT EXECUTION / "
    "PAID REAL BACKEND / PREDECLARED TECHNICAL GATES"
)
DRY_RUN_BANNER = (
    "NON-FORMAL / PHASE 13D CANONICAL EPISODE PRODUCER DRY RUN / "
    "ZERO-LLM / NOT FORMAL EXPERIMENT EVIDENCE"
)
POOL_FINALIZE_BANNER = (
    "FORMAL ASSET FREEZE / PHASE 13D CANONICAL EPISODE POOL FINALIZATION / ZERO-LLM"
)


def default_producer_contract_path() -> Path:
    return Path(__file__).with_name("producer_contract_v1.json")
...
def load_producer_contract(path: str | Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise CanonicalEpisodeProducerError(f"cannot load Phase 13D producer contract: {source}") from exc
    if not isinstance(payload, dict):
        raise CanonicalEpisodeProducerError("Phase 13D producer contract root must be an object")
...
def validate_producer_contract(contract: Mapping[str, Any]) -> None:
    if _canonical_json_sha256(contract) != PRODUCER_CONTRACT_SHA256:
        raise CanonicalEpisodeProducerError("Phase 13D producer-contract SHA-256 drifted")
    if contract.get("contract_schema_version") != "marketlens-canonical-episode-producer-contract/1.0":
        raise CanonicalEpisodeProducerError("Phase 13D producer schema drifted")
    if contract.get("contract_version") != PRODUCER_CONTRACT_VERSION:
        raise CanonicalEpisodeProducerError("Phase 13D producer version drifted")
    if contract.get("status") != PRODUCER_CONTRACT_STATUS:
        raise CanonicalEpisodeProducerError("Phase 13D producer status drifted")
    if contract.get("phase13c_execution_plan_sha256") != EXPECTED_EXECUTION_PLAN_SHA256:
        raise CanonicalEpisodeProducerError("Phase 13D no longer binds exact Phase 13C v1.2 plan")
    if contract.get("episode_pool_id") != EPISODE_POOL_ID:
        raise CanonicalEpisodeProducerError("Phase 13D episode-pool identity drifted")
    if tuple(contract.get("episode_ids", ())) != EPISODE_IDS:
        raise CanonicalEpisodeProducerError("Phase 13D episode slot list drifted")
    ...
    if controls != expected_controls:
        raise CanonicalEpisodeProducerError("Phase 13D execution-control policy drifted")
...
def validate_runtime_dependencies(
    *,
    ...
    if execution_plan_sha256(plan) != contract["phase13c_execution_plan_sha256"]:
        raise CanonicalEpisodeProducerError("active Phase 13C plan does not match Phase 13D producer contract")
...
def dry_run_summary(
    *,
    ...
    return {
        ...
        "full_pool_execute_command_allowed": False,
        "phase13c_execution_plan_sha256": deps["execution_plan_sha256"],
...
def _planned_calendar(repo_root: Path, frozen_days: Sequence[Mapping[str, Any]]) -> tuple[Any, ...]:
    ...
    if len(calendar) != len(frozen_days):
        raise CanonicalEpisodeProducerError("runtime calendar length drifted from frozen Phase 13C plan")
...
def execute_formal_episode_slot(
    *,
    ...
    if git["branch"] != "dissertation":
        raise CanonicalEpisodeProducerError(
            f"formal slot execution must run on dissertation branch, got {git['branch']!r}"
        )
    ...
    if (root / "data/marketlens/canonical_episode/v1/pool_manifest.json").exists():
    ...
    attempt_base: dict[str, Any] = {
        "manifest_schema_version": "marketlens-canonical-episode-attempt/1.0",
        "episode_pool_id": EPISODE_POOL_ID,
        "episode_id": episode_id,
        "attempt_number": attempt_number,
        "git": git,
        "phase13c_execution_plan_sha256": execution_plan_sha256(plan),
        "phase13d_producer_contract_sha256": PRODUCER_CONTRACT_SHA256,
        "backend": {
...
def finalize_formal_episode_pool(*, repo_root: str | Path) -> dict[str, Any]:
    ...
    plan = load_execution_plan()
    pool_path = root / "data/marketlens/canonical_episode/v1/pool_manifest.json"
    if pool_path.exists():
