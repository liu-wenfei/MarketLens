"""Phase 9C N20 three-calendar-day real-backend validation.

This module is a thin validation runner built on already-frozen MarketLens
boundaries. It reuses the existing verified N20 fixture and the exact Phase 9B
deterministic activation trajectory. It does not regenerate a population or
search for a more convenient activation seed.
"""
...
PHASE09C_N20_VERSION = "marketlens_phase09c_n20_multiday_validation/1.0"
BANNER = (
    "NON-FORMAL / N20 THREE-DAY REAL-BACKEND VALIDATION / "
    "NOT FORMAL EXPERIMENT EVIDENCE"
)
DRY_BANNER = (
    "NON-FORMAL / N20 THREE-DAY DRY RUN / 0 LLM / "
    "NOT FORMAL EXPERIMENT EVIDENCE"
)
...
DEFAULT_RUNTIME_DB = (
    "artifacts/preflight/phase05b/dev_population_n20/population_runtime.db"
)
DEFAULT_POPULATION_MANIFEST = (
    "artifacts/preflight/phase05b/dev_population_n20/population_manifest.json"
)
...
# Observed by Phase 9B before choosing N20 for the paid multi-day validation.
# This is a reproducibility/drift guard, not seed fishing.
EXPECTED_ACTIVE_IDS = (
    (
        "42710178889",
        "70690674622",
...
def _verify_reference_fixture(
    *, runtime_db: Path, population_manifest: Path
) -> tuple[str, str]:
    if not runtime_db.is_file():
        raise Phase09CError(f"verified N20 runtime fixture not found: {runtime_db}")
    if not population_manifest.is_file():
        raise Phase09CError(
            f"verified N20 population manifest not found: {population_manifest}"
        )
    ...
    if runtime_sha != EXPECTED_RUNTIME_SHA256:
        raise Phase09CError(
            "N20 runtime fixture hash differs from the previously verified "
            f"Phase 3/5 fixture: {runtime_sha}"
        )
    if manifest_sha != EXPECTED_MANIFEST_SHA256:
        raise Phase09CError(
            "N20 population manifest hash differs from the previously verified "
            f"Phase 3/5 manifest: {manifest_sha}"
        )
    return runtime_sha, manifest_sha
...
def _build_plan(*, trading_days: frozenset[str]) -> tuple[Any, ...]:
    ...
    if tuple(day.current_date for day in plan) != EXPECTED_DATES:
        raise Phase09CError("N20 validation calendar horizon drifted")
...
def _activation_sequence(
    *, runtime_db: Path, plan: Sequence[Any]
    ...
    if len(profiles) != POPULATION_SIZE:
        raise Phase09CError(
            f"verified N20 fixture produced {len(profiles)} activation profiles"
        )
    ...
    for index, item in enumerate(sequence):
        ...
        if actual_ids != expected_ids:
            raise Phase09CError(
                "Phase 9B deterministic N20 activation reference drifted on "
        ...
        if output_digest is None:
            raise Phase09CError("unexpected null activation-state digest")
        evidence.append(
            {
                "step": item.day.step,
...
def validate_n20_real_summary(
    ...
    if summary.get("population", {}).get("size") != POPULATION_SIZE:
        failures.append("population size is not fixed N20")
    ...
    if not integrity.get("verified_n20_fixture_unchanged"):
        failures.append("verified N20 fixture changed")
    ...
    if not continuity.get("all_graphs_bounded_n20"):
        failures.append("one or more daily graphs were not bounded N20")
...
def run_phase09c_n20(
    *,
    ...
    run_id = (
        ...
        + f"_{git['commit'][:8]}_phase09c_n20_{mode}"
    )
    run_dir = artifact_root_p / run_id
    run_dir.mkdir(parents=True, exist_ok=False)

    fixture_runtime_sha, fixture_manifest_sha = _verify_reference_fixture(
    ...
    if len(population_ids) != POPULATION_SIZE:
        raise Phase09CError(
            f"verified fixture has {len(population_ids)} Agents, expected N20"
        )
    ...
    protected_paths = {
        ...
        "verified_n20_runtime_db": runtime_db_p,
        "verified_n20_population_manifest": manifest_p,
    ...
    summary: dict[str, Any] = {
        ...
        "phase": "9C-N20",
        "gate_version": PHASE09C_N20_VERSION,
        ...
        "population": {
            ...
            "fixture_origin": "existing verified Phase 3/5 N20 development fixture",
            ...
            "regenerated_for_phase09c": False,
        },
        "activation": {
            "seed": ACTIVATION_SEED,
            "policy": "frozen Phase 4",
            "reference_origin": (
                "Phase 9B zero-LLM sequence observed before N20 paid "
                "validation decision"
            ),
    ...
    workspace_ctx = tempfile.TemporaryDirectory(prefix="marketlens_phase09c_n20_")
        ...
        summary.update(
            {
                ...
                "continuity": {
                    ...
                    "all_graphs_bounded_n20": all_graphs_bounded_n20,
